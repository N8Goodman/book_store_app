...
feature "Admin revokes user's admin privledges" do
  ...
  scenario "Admin updates non-admin user to be an admin" do
    ...
    expect(page).to_not have_link("make-admin#{non_admin_user2.id}")

    click_on "revoke-admin#{non_admin_user2.id}"

    expect(page).to have_content "#{non_admin_user2.user_name} is no longer an admin."
    expect(page).to have_link("make-admin#{non_admin_user2.id}")
